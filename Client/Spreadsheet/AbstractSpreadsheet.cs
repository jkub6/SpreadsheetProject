...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            visited.Add(name);
            foreach (String n in GetDirectDependents(name)) //loop through new connections
            {
                if (n.Equals(start)) //assure we havent visited starting node
                {
                    throw new CircularException();
                }
                else if (!visited.Contains(n)) //if we haven't been there yet, expore new connection recursively
                {
